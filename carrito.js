...
function actualizarTotal() {
    ...
    let productoEnCarrito =
        contenedorCarrito.getElementsByClassName("producto-carrito");
    let total = 0;

    for (let i = 0; i < productoEnCarrito.length; i++) {
        let producto = productoEnCarrito[i];
        let precioProducto = producto.getElementsByClassName(
            "precio-producto-carrito"
        )[0];
        console.log(precioProducto);

        let precio = parseFloat(
            precioProducto.innerText.replace("$", "").replace(".", "")
        );
        console.log(precio);

        let cantidadProducto = producto.getElementsByClassName(
    ...
    document.getElementsByClassName("precio-total-carrito")[0].innerText =
...
function sumarProducto(event) {
    ...
    let cantidadProducto = selector.getElementsByClassName(
        "cantidad-producto-carrito"
    )[0];
    let cantidadActual = parseInt(cantidadProducto.value);
    console.log(cantidadActual);
...
function agregarAlCarro(event) {
    ...
    let nombreProducto =
        producto.getElementsByClassName("nombre-producto")[0].innerText;
    console.log(nombreProducto);
    let precio = producto.getElementsByClassName("precio-producto")[0].innerText;
    let imagenProducto =
        producto.getElementsByClassName("imagen-producto")[0].src;
...
function agregarProductoAlCarro(nombreProducto, precio, imagenProducto) {
    ...
    let nombreProductoCarrito = itemsCarrito.getElementsByClassName(
    ...
    for (let i = 0; i < nombreProductoCarrito.length; i++) {
        if (nombreProductoCarrito[i].innerText == nombreProducto) {
            Swal.fire(
                ...
                "puedes modificar la cantidad e el selector.",
                "success"
            );
            return;
        }
